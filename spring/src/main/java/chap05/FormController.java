...
public class FormController {
	...
	/*
	 * 파라미터를 받는 방법
	 * 1. HttpServletRequest 객체로 받는방법 
	 * 2. @RequestParam 으로 받는 방법
	 * 3. 커맨드객체 
	 * - 파라미터의 이름과 동일한 필드에 자동으로 값을 넣어줌
	 */
	@PostMapping("/send.do")
	public String send(HttpServletRequest req, @RequestParam("email") String email,
			@RequestParam(value="tel", required = false )String tel, MemberVo vo){
		...
		//커맨드객체없이 취미값을 vo의 hobby에 저장하려면?
		MemberVo vo2 = new MemberVo();
		vo2.setHobby(req.getParameterValues("hobby"));
//		String []hobbys = new String [req.getParameter("hobby").length()];
