  ...
  <script>
  	function formCheck() {
		...
		$.ajax({
	  			...
	  			method:"get", // UserController에서 idcheck 부분을 postmapping으로 했으면 post로 해야함 
	  			success: function(r) {
	  				//console.log(r);
	  				if(r.trim() =='1') {
	  					alert ('아이디가 중복 되었습니다. 다른아이디를 입력하세요.');
	  					$("#id").val('');
	  					$("#id").focus();
	  					//return false;
	  					con=false;
  		...
  		//학교정보체크
  		$("input[name='school']").each(function(idx){ //input태그의 name 속성의 값이 school 선택자의 객체 반복
