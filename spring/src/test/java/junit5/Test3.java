...
public class Test3 {
	...
	void boardInsert() throws Exception{
		File f = new File("C:\\Users\\82103\\Desktop\\K-Digital\\Web\\img\\logo.png");
		FileInputStream fis = new FileInputStream(f);
		MockMultipartFile file = new MockMultipartFile("file", f.getName(), "multipart/form-data",fis);
		RequestBuilder req = MockMvcRequestBuilders.multipart("/board/insert.do").
				file(file).param("title", "제목테스트").param("content", "내용테스트").param("writer", "작성자테스트").session(session);
		mockMvc.perform(req);
	}
